...
+
Cloud Manager に新しい AWS クレデンシャルを追加すると、 ONTAP 作業環境の FSX の作成と管理、またはコネクタの作成に必要な権限が Cloud Manager に付与されます。 <<Add credentials to Cloud Manager,Cloud Manager に AWS クレデンシャルを追加する方法について説明します>>。
